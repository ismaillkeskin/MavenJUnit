...
public class C01_WindowHandles extends TestBase {
    ...
    handle degerini bilmedigim icin normal getWindowHandle ile driver'imi yeni pencereye geciremem bunun icin
    getWindowHandles() methoduyla acilan tum pencereleri bir Set'e assign edip, ilk açtığımız pencere değilse ikinci açılan yeni
    penceredir mantığıyla bir loop içinde çözebiliriz
     */
       Set<String> pencereler = driver.getWindowHandles();
        for (String w:pencereler) {
            System.out.println(w);
          if (!w.equals(ilkSayfaHandle)){ //223B02065519B09BA79B82CEE5423C32
              driver.switchTo().window(w); //A1543B013014E7A21DA9197C4A5E9D1E
          }
        }
